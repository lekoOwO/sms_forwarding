...
- 按来源号码或内容配置转发规则；可以用手机编辑 CSV、测试规则，并自定义通知标题与正文。
- 在管理页查看、发送短信，查询信号、SIM 与模组信息；诊断原值可以点按或用键盘展开。
- 保存最多五组 WiFi，配置心跳通知，导出加密配置备份。
- 配置 HTTP 或 HTTPS 移动网络保活下载，HTTPS 证书可在管理页配置。
...
管理页提供交互式 CSV 编辑与语法检查。输入来源号码与短信文字即可测试规则。
测试不会保存配置或发送通知；Demo 结果只供演示。
旧 Tab 转发规则仍可使用。管理页可以将规则转换为 CSV，确认内容后再保存。

### 移动传送与保活

电子邮件只通过 WiFi 发送。使用 4G 推送前，请确认移动网络可用，并启用通道的移动网络功能与 HTTPS 证书。目前不支持漫游时的 4G 推送。实际可用性取决于模组、SIM 卡和网络。

移动网络保活会使用 SIM 数据，可能产生费用；需要兼容模组与本地网络注册。
保活支持 HTTP 与 HTTPS。HTTP 未加密，网址请勿包含敏感信息；HTTPS 需先配置目标证书。
HTTPS 保活有独立的证书配置入口，不需要创建推送通道。
