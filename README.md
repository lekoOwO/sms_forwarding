...
| `app0` | 1.875MB | OTA 固件槽位 0 |
| `app1` | 1.875MB | OTA 固件槽位 1 |
| `appcfg` | 128KB | 应用配置 NVS |
| `coredump` | 64KB | 崩溃转储，便于定位无人值守设备的异常 |

`app0`/`app1` 保留双 OTA 槽位，每个槽位 1.875MB。收件箱使用运行时内存，不占用独立持久化分区；构建后的镜像大小和 OTA 槽位余量由 `tools/check_idf_baseline.py` 输出并校验。
