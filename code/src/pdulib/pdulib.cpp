...
PDU::~PDU() { delete[] generalWorkBuff; }
...
int PDU::convert_7bit_to_unicode(unsigned char *gsm7bit, int length, char *unicode)
{
  ...
  w = 0;
  auto appendCodePoint = [&](unsigned long codepoint) {
    char encoded[5];
    int encodedLength = buildUtf(codepoint, encoded);
    if (w + encodedLength >= generalWorkBuffLength) {
      overFlow = true;
      return false;
    }
    memcpy(&unicode[w], encoded, encodedLength);
    w += encodedLength;
    return true;
  };
  for (r = 0; r < length; r++)
  {
    ...
    if ((lookup_gsm7toUnicode[(unsigned char)gsm7bit[r]]) != 27)
    {
      ...
      */
      ...
      if (gsm7bit[r] == '?' || C != NPC8) {
        if (!appendCodePoint(C)) break;
      }
      else if (gsm7bit[r] >= 16 && gsm7bit[r] <= 26)
      {
#ifdef PM
...
#endif
        if (!appendCodePoint(S)) break;
      }
      else if (!appendCodePoint(NPC8)) break;
    }
    else
    {
      /* If we're escaped then the next uint8_t have a special meaning. */
      if (++r >= length) {
        if (!appendCodePoint(NPC8)) break;
        break;
      }
      switch (gsm7bit[r])
      {
      case 10:
        if (!appendCodePoint(12)) r = length;
        break;
      case 20:
        if (!appendCodePoint('^')) r = length;
        break;
      case 40:
        if (!appendCodePoint('{')) r = length;
        break;
      ...
      case 0x65:
        // unicode[w++] = '€';  // euro
        if (!appendCodePoint(0x20AC)) r = length;
        break;
      default:
        if (!appendCodePoint(NPC8)) r = length;
        break;
      }
    }
  }
...
int PDU::pduGsm7_to_unicode(const char *pdu, int numSeptets, char *unicode, char firstchar)
{
  ...
  int w;
  int length;
  if (numSeptets < 0 || numSeptets > MAX_SMS_LENGTH_7BIT) return -1;
  unsigned char gsm7bit[MAX_SMS_LENGTH_7BIT + 1];
...
{
  return pdu && decodePDU(pdu, strlen(pdu));
}

static int packedBytesRead(int septets, bool hasFirstCharacter)
{
  int produced = hasFirstCharacter ? 1 : 0;
  int bytes = 0;
  while (produced < septets) {
    produced += bytes % 7 == 6 ? 2 : 1;
    bytes++;
  }
  return bytes;
}

bool PDU::decodePDU(const char *pdu, size_t pduLength)
{
  bool rc = false;
  size_t index = 0;
  int outindex = 0;
  ...
  if (!pdu || pduLength == 0 || (pduLength & 1) != 0 ||
      pduLength > PDU_BINARY_MAX_LENGTH * 2) return false;
  for (size_t position = 0; position < pduLength; position++)
    if (!isxdigit((unsigned char)pdu[position])) return false;
  auto require = [&](size_t chars) {
    return index <= pduLength && chars <= pduLength - index;
  };

  overFlow = false;
  memset(concatInfo, 0, sizeof(concatInfo));
  *generalWorkBuff = *scabuffin = *addressBuff = *tsbuff = 0;
  i = decodeAddress(pdu, pduLength, scabuffin, OCTETS);
  if (i < 0) return false;
  index = i;
  if (!require(2)) return false;
  tpdu = gethex(&pdu[index]);
  index += 2; // skip over tpdu
  if ((tpdu & 3) != PDU_SMS_DELIVER) return false;
  udhPresent = tpdu & (1 << PDU_UDHI);
  i = decodeAddress(&pdu[index], pduLength - index, addressBuff, NIBBLES);
  if (i < 0) return false;
  index += i;
  if (!require(4 + 14 + 2)) return false;
  ...
  *generalWorkBuff = 0;
  if (udhPresent)
  {
    ...
    index += 2;
    if ((udhlength != 5 && udhlength != 6) || !require(udhlength * 2)) return false;
    switch (udhlength)
    {
    default:
      return false;
    ...
    case 6:
      ...
      if ((udhlength == 5 && iei == 0 && ieilength == 3) || (udhlength == 6 && iei == 8 && ieilength == 4))
      {
        ...
        {
          int headerSeptets = udhlength == 5 ? 7 : 8;
          if (dulength < headerSeptets) return false;
          dulength -= headerSeptets;
          if (udhlength == 5) {
          ...
          if (dulength < udhlength + 1) return false;
  ...
  case DCS_7BIT_ALPHABET_MASK:
    if (dulength > MAX_SMS_LENGTH_7BIT ||
        !require((size_t)packedBytesRead(dulength, udhfollower != 0) * 2)) return false;
    outindex = 0;
  ...
  case DCS_16BIT_ALPHABET_MASK:
    if ((dulength & 1) != 0 || !require((size_t)dulength * 2)) return false;
    ...
    while (dulength)
    {
      ...
      dulength -= 2;
      char utf8[4];
      utflength = ucs2_to_utf8(ucs2, utf8);
      if ((utfoffset + utflength) >= generalWorkBuffLength) {
        overFlow = true;
        break;
      }
      memcpy(generalWorkBuff + utfoffset, utf8, utflength);
      utfoffset += utflength;
    }
...
*/
int PDU::decodeAddress(const char *pdu, size_t available, char *output, eLengthType et)
{                           // pdu to readable starts with length octet
  if (available < 2) return -1;
  int length = gethex(pdu); // could be nibbles or octets
  if (et == OCTETS && length == 0) {
    *output = 0;
    return 2;
  }
  addressLength = et == NIBBLES ? length : (length - 1) * 2;
  size_t consumed = et == NIBBLES ? 4 + ((addressLength + 1) / 2) * 2
                                   : 2 + length * 2;
  if (addressLength < 1 || addressLength > MAX_NUMBER_LENGTH || consumed > available)
    return -1;
  ...
  if ((adt & EXT_MASK) != 0)
  {
    ...
    case 1:            // international number
      if (addressLength >= MAX_NUMBER_LENGTH) return -1;
    ...
    case 5: // alphabetic, convert  nibble length to septets
      if (pduGsm7_to_unicode(pdu, (addressLength * 4) / 7, generalWorkBuff, 0) < 0 ||
          strlen(generalWorkBuff) > MAX_NUMBER_LENGTH) return -1;
      strcpy(output, generalWorkBuff);
      if ((addressLength & 1) == 1) // if odd, bump 1
