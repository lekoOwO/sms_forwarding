...
def render_web(schema: dict, manifest: dict) -> str:
    ...
    fields = [
        ...
        ("notificationLocale", config["notificationLocale"]),
        ("smtpServer", config["smtpServer"]),
        ("smtpUser", config["smtpUser"]),
        ("smtpPass", config["smtpPass"]),
        ("smtpSendTo", config["smtpSendTo"]),
        ("adminPhone", config["adminPhone"]),
        ("numberBlackList", config["numberBlackList"]),
        ...
        ("pushBodyTemplate", channel["bodyTemplate"]),
        ("pushCustomBody", channel["customBody"]),
    ]
    field_lines = "\n".join(
        f'  "{name}": {node["x-maxUtf8Bytes"]}{"," if index < len(fields) - 1 else ""}'
        for index, (name, node) in enumerate(fields)
    )
    return f"""// Generated by tools/generate-config-schema.py. Do not edit.
export const CONFIG_SCHEMA_VERSION = {root['schemaVersion']['const']} as const;
export const CONFIG_MIME_TYPE = {c_string(manifest['mimeType'])} as const;
export const BACKUP_ENVELOPE = {{
  magic: {c_string(envelope['magic'])},
  ...
  byteOrder: {c_string(envelope['byteOrder'])},
  aad: {c_string(envelope['aad'])},
...
export const CONFIG_VALUE_LIMITS = {{
  networkMode: {{ min: {config['networkMode']['minimum']}, max: {config['networkMode']['maximum']}, default: {config['networkMode']['default']} }},
  heartbeatInterval: {{ min: {config['heartbeatInterval']['minimum']}, max: {config['heartbeatInterval']['maximum']}, default: {config['heartbeatInterval']['default']} }}
}} as const;
export const NOTIFICATION_LOCALES = {json.dumps(config['notificationLocale']['enum'], ensure_ascii=False)} as const;
"""
...
def main() -> int:
    ...
    expected_firmware = render_firmware(schema, manifest)
    expected_web = render_web(schema, manifest)
    if args.check:
        outputs = ((FIRMWARE_OUTPUT, expected_firmware), (WEB_OUTPUT, expected_web))
        for path, expected in outputs:
            ...
            return 1
        return 0
    FIRMWARE_OUTPUT.write_text(expected_firmware, encoding="utf-8")
    WEB_OUTPUT.write_text(expected_web, encoding="utf-8")
    return 0
