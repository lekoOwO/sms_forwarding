...
class FirmwareVersionTests(unittest.TestCase):
    def test_checked_in_version_source_and_header_are_synchronized(self):
        ...
        self.assertEqual({"releaseVersion": "1.1.4", "devBuild": 15}, version)
        header = HEADER.read_text(encoding="utf-8")
        self.assertIn('#define FIRMWARE_RELEASE_LABEL "1.1.4 (15)"', header)
        self.assertIn('#define FIRMWARE_DEV_BUILD_TEXT "15"', header)
    ...
    def test_build_mode_selects_the_public_firmware_version(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            for release_mode, expected in (("0", "15"), ("1", "1.1.4 (15)")):
