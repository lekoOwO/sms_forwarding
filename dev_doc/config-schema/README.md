...
CFG2 payload is 26,520 bytes; with its 20-byte header the encoded binary is
26,540 bytes, leaving 6,228 bytes under the 32,768-byte limit. Push template
alternatives are counted exclusively because custom channels use `customBody`
instead of title/body templates.

Portable restore marks target-local identity, accounts, hostname, WiFi/SIM
credentials, SIM failure counters, phone number, and runtime profile/timestamp
fields with `x-portableRestore: false`. `wifiFromFallback` is runtime-only and
is intentionally absent. Provider values 1 through 12 are supported; value 0
