...
|---|---|---|
| `e113dd3` — move the build to ESP-IDF 6.0.2 | `.github/workflows/build.yml`, `CMakeLists.txt`, `sdkconfig.defaults`, and `tools/idf.ps1` use the current IDF 6 path. The workflow also runs the activation, install, eSIM Web, and native TLS checks. The current local `./tools/idf.sh build` passed, and `python3 tools/check_idf_baseline.py --build-dir build/idf` passed: firmware `1,576,032` bytes with `390,048` bytes of OTA headroom. Historical CI run `34694717521` passed lint, mock, and firmware build. | `covered` for the pinned local build baseline. The historical run and this local build do not prove a new GitHub CI run. GitHub CI remains pending. |
...
| `2904be3` — add the eSIM profile download flow and activation-code parser | Current split modules are `idf_esim`, `idf_lpa_rsp`, `idf_lpa_install`, `idf_lpa_bpp`, and `idf_lpa_es9_transport`. The parser accepts three to seven fields. `test_activation_code.py`, `test_install.py`, and `test_esim_install.py` cover validated optional fields and fail-closed malformed input. | `covered` for the parser and installer boundary. The tests do not assign provider-specific meaning to optional fields. |
...
| `59d86cc` — refine eSIM download, notification, and diagnostic behavior | Current code sends unfiltered `BF 2B 00`, matching the upstream change. BPP metadata remains verified before the final card write, and valid BPP-first input is covered. Generic notification parsing, ordering, duplicate policy, and deferred host groups are covered by the LPA fixtures. | `partial`. The scoped download and notification behavior is covered. Per-download counts and heap fields, plus bounded `subjectCode` and `reasonCode` fields, are still being integrated. The latest ES9 truncated-count regression still fails. |
...
| Activation optional fields | The parser accepts three to seven `$` fields. The installer now accepts validated optional fields and still rejects malformed input. | `test_activation_code.py`, `test_install.py`, and `test_esim_install.py` cover seven-field success and fail-closed malformed input. They do not assign provider-specific OID semantics. | Covered at the parser and installer boundary. |
